...
Each use case names the harness that serves it. A harness is whichever mechanism does the work: a GitHub Actions workflow, the player page, a chat command, or a repo file. Nothing here is custom infrastructure.

## Harness catalog

| Harness | What it is | State it touches |
|---|---|---|
| H1 daily-digest skill | On-demand `/digest` command (`routines/daily.md`); one pass per enabled front | reads each front's `sources.md`, writes its `digests/`, triggers player deployment |
| H2 deep-dive skill | On-demand `/deepdive` command (`routines/weekly.md`); one front per run, round-robin | reads that front's `curriculum.md`, writes its `digests/`, checks item off, triggers player deployment |
| H3 player page | GitHub Pages site on your phone: front switcher, TTS playback, progress, listened state and star ratings (localStorage seeded from `log.md` at build; sync button emits a paste-string) | on-device only |
| H4 chat ops | Ad-hoc AI agent session: "mark X listened", "rate X 4", pasted `mark listened: <ids>` / `rate: <id>=<n>` sync strings, "add source Y", "add front Z", "promote W to curriculum" | `log.md`, `fronts/` |
| H5 spark hook | Button in the player that opens a **new, external AI chat session** prefilled with the episode context | none — fire and forget |
...
- **Repo → devices:** `build.py` bakes the checked ids from `log.md` into the player; each device applies them as listened on load (once per id, so a local un-check for replay sticks). H4 rebuilds and redeploys right after updating `log.md`, so the state reaches devices on the next player reload (H1's rebuild when generating content also picks it up).
- **Devices → repo:** episodes finished locally but unchecked in `log.md` surface a **sync N to log ⇪** pill in the player header; tapping copies `mark listened: <ids>` to the clipboard — paste it into any AI chat and H4 updates `log.md`.
The player still never writes anywhere (static hosting has no backend — that's why the paste hop exists). H1 checks the pending count before generating and shortens episodes if the backlog is growing.

### UC4 — Manage sources
Edit a front's `sources.md` directly, or ask your AI agent (H4). Takes effect next workflow run. Disable without deleting by prefixing a line with `x`.

### UC5 — Spark: an episode wakes up an idea
While listening, tap **Spark** on the episode. It opens a new AI chat session (your preferred assistant) prefilled with the episode title, its key context, and your cue to deep-dive. That session is launched and managed **outside this solution** — the only responsibility here is firing it. Nothing is written to the repo; if the external session produces something worth keeping, bring it back via H4 (e.g. "promote this to curriculum Track E").

### UC6 — Choose the listening language
Sources and canonical episodes are always English; if `config.md` sets a secondary language, content generation (H1/H2) emits a pre-translated rendition (`.<lang>.md`, technical terms kept in English) at creation time — the player cannot translate live (static hosting has no backend). Language resolution, highest precedence first: (1) per-episode pin — tap the language chip on a card to switch that episode to the other language, tap again to unpin (pinned = accent color + ✱); (2) the global AUTO/EN/ES toggle in the transport bar; (3) AUTO default = news in the secondary language, deep-dives in English. Episodes without a rendition always play in English. Position, voices, and duration adjust per language; listening position survives a language switch (paragraph mapping). **Harness:** H1/H2 translate, H3 plays.
...
2. If it needs a new harness, prefer, in order: a repo file convention (free), a player feature (H3, still no backend), a chat op (H4), a new on-demand skill (H1/H2 pattern).
