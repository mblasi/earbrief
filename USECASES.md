...
|---|---|---|
| H1 daily-digest workflow | GitHub Actions workflow, daily (`routines/daily.md`); one pass per enabled front | reads each front's `sources.md`, writes its `digests/`, triggers player deployment |
| H2 deep-dive workflow | GitHub Actions workflow, weekly (`routines/weekly.md`); one front per run, round-robin | reads that front's `curriculum.md`, writes its `digests/`, checks item off, triggers player deployment |
...
The player still never writes anywhere (static hosting has no backend — that's why the paste hop exists). Weekly workflow flags a growing pending pile and shortens episodes if needed.
...
2. If it needs a new harness, prefer, in order: a repo file convention (free), a player feature (H3, still no backend), a chat op (H4), a new workflow (H1/H2 pattern — costs a schedule).
