...
- [ ] E10. Kimi K3's Kimi Delta Attention and Attention Residuals: how a linear-attention recurrent state combined with depth-wise residual retrieval lets a 2.8T-parameter, 896-expert mixture-of-experts model activate only 16 experts (~50B parameters) per token, why quantization-aware training from the SFT stage onward lets MXFP4 weights ship without the usual post-hoc quality collapse, and how this recurrent-state bet compares to DeepSeek V4's sparse-retrieval approach to the same long-context cost problem (2026-07-26)
